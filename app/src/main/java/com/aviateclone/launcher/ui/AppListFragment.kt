...
class AppListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        listAdapter = AppListAdapter { app -> (activity as? MainActivity)?.launchApp(app) }
        b.rvAllApps.apply {
            adapter = listAdapter
            layoutManager = LinearLayoutManager(requireContext())
        }
        vm.allApps.observe(viewLifecycleOwner) { apps -> listAdapter.submitApps(apps) }
        b.etSearch.addTextChangedListener(object : TextWatcher {
            override fun beforeTextChanged(s: CharSequence?, st: Int, c: Int, a: Int) {}
            override fun onTextChanged(s: CharSequence?, st: Int, b2: Int, c: Int) {
                listAdapter.filter(s?.toString() ?: "")
                b.rvAllApps.scrollToPosition(0)
            }
